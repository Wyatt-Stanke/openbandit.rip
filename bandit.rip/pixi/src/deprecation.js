...
export default function deprecation(core) {
  const { mesh, particles, extras, filters, prepare, loaders, interaction } =
    core;
  ...
  Object.defineProperties(core, {
    ...
    AssetLoader: {
      get() {
        throw new ReferenceError(
          "The loader system was overhauled in PixiJS v3, " +
            "please see the new PIXI.loaders.Loader class."
        );
      },
    },
  ...
  for (let i = 0; i < defaults.length; i++) {
    ...
    Object.defineProperty(core[deprecation.parent], "DEFAULT", {
      get() {
        warn(
          `PIXI.${deprecation.parent}.DEFAULT has been deprecated, ` +
  ...
  core.DisplayObject.prototype.generateTexture = function generateTexture(
    renderer,
    scaleMode,
    resolution
  ) {
  ...
  core.RenderTexture.prototype.render = function render(
    displayObject,
    matrix,
    clear,
    updateTransform
