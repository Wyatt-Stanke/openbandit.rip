...
export default class MeshRenderer extends core.ObjectRenderer {
  ...
  render(mesh) {
    ...
    renderer.state.setBlendMode(
      core.utils.correctBlendMode(
        mesh.blendMode,
    ...
    if (glData.shader.uniforms.uTransform) {
      if (mesh.uploadUvTransform) {
        glData.shader.uniforms.uTransform =
          mesh._uvTransform.mapCoord.toArray(true);
      } else {
    ...
    glData.shader.uniforms.uColor = core.utils.premultiplyRgba(
      mesh.tintRgb,
      mesh.worldAlpha,
    ...
    const drawMode =
      mesh.drawMode === Mesh.DRAW_MODES.TRIANGLE_MESH
        ? gl.TRIANGLE_STRIP
        : gl.TRIANGLES;
