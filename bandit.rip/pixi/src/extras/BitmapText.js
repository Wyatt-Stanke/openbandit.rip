...
export default class BitmapText extends core.Container {
  ...
  updateText() {
    ...
    for (let i = 0; i < lenChars; i++) {
      ...
      c.position.x =
        (chars[i].position.x + lineAlignOffsets[chars[i].line]) * scale;
  ...
  set font(
    ...
    if (typeof value === "string") {
      ...
      this._font.size =
        value.length >= 2
          ? parseInt(value[0], 10)
          : BitmapText.fonts[this._font.name].size;
    } else {
      this._font.name = value.name;
      this._font.size =
        typeof value.size === "number" ? value.size : parseInt(value.size, 10);
    }
