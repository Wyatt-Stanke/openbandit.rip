...
export default class ParticleRenderer extends core.ObjectRenderer {
  ...
  render(container) {
    ...
    if (!buffers) {
      buffers = container._glBuffers[renderer.CONTEXT_UID] =
    ...
    this.shader.uniforms.uColor = core.utils.premultiplyRgba(
      container.tintRgb,
  ...
  generateBuffers(container) {
    ...
    for (let i = 0; i < size; i += batchSize) {
      buffers.push(
        new ParticleBuffer(gl, this.properties, dynamicPropertyFlags, batchSize)
      );
    }
